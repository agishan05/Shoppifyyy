...
function ProductCard({ product, onAddToCart, onToggleWishlist, isWishlisted, highlightText }) {
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[1.5rem] border border-black/5 bg-white shadow-[0_12px_30px_rgba(0,0,0,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(0,0,0,0.14)]">
      <div className="relative overflow-hidden">
        ...
        <button
          ...
          className={`absolute right-4 top-4 rounded-full p-2 transition ${isWishlisted ? 'bg-yellow-400 text-black' : 'bg-white/90 text-gray-700 hover:bg-yellow-400'}`}
        >
          ♥
        </button>
      </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="mb-3 flex items-center justify-between gap-3 text-sm text-gray-500">
          <span className="rounded-full bg-gray-100 px-2.5 py-1">{product.category}</span>
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <span className="rounded-full bg-green-50 px-3 py-1 text-sm font-medium text-green-700">{product.availability}</span>
        </div>
